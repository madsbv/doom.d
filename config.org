...
*** lua
#+begin_src emacs-lisp
(setq! lsp-clients-lua-language-server-install-dir (getenv "LUA_LANGUAGE_SERVER_INSTALL_DIR"))
#+end_src
*** csv-mode
:PROPERTIES:
...
#+end_src
*** html
`ws-butler-mode` formats/removes unnecessary whitespace. Further, `before-save-hook` by default includes `ws-butler-before-save`, which triggers on save whether the mode is enabled or not.
This is an issue for Askama HTML templating.
#+begin_src emacs-lisp :tangle yes
(appendq! ws-butler-global-exempt-modes '(html-mode))
#+end_src
...
(use-package! gptel
  ...
  (setq! gptel-model "llama3:latest"
         gptel-backend (gptel-make-ollama "mbv-desktop"
                                          :host "mbv-desktop:11434"
                                          :stream t
                                          :models '("llama3:latest" "dolphin-mixtral:latest" "phi3"))))
#+end_src
** ledger
:PROPERTIES:
