...
(use-package! org-gtd
  ...
  (map! :leader
        ...
        (:prefix ("d m" . "org-gtd-mu4e")
         :desc "Capture mail" "c" #'mu4e-org-gtd-store-and-capture
         :desc "Delegate task via mail" "d" #'mu4e-org-gtd-email-reminder)))
#+end_src
**** Automation/email integration
:PROPERTIES:
:ID:       11043675-99ef-410e-8a47-bea909776e21
:END:
This is an example from the org-gtd manual showing how to integrate. This particular function is not exactly what we want, but could be a useful template.
#+begin_src emacs-lisp :tangle no
  ...
  (defun org-gtd-email-reminder ()
    (let ((subject (message-fetch-field "subject"))
          ...
          (link (mu4e-org-store-link))
          (reminder-date (org-read-date nil nil "+3d")))
      (org-gtd-delegate-create (format "Reminder: %s, %s" topic link)
                               delegated-to
                               reminder-date)))
