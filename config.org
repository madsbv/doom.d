...
(package! org-ref)
;; Package for managing scheduled jobs on MacOS (launchctl is modern CRON). Superseded by LaunchControl app.
;(package! launchctl)
#+end_src
...
Set "echo 'allow-loopback-pinentry' >> .gnupg/gpg-agent.conf && gpgconf --reload gpg-agent". Note also that there is currently a bug in Emacs integration with gpg version 2.4.x for x>0 (https://dev.gnupg.org/T6481), seems that there's a patch on the way though. Currently (231130) I have gpg pinned to version 2.4.0 in Homebrew.
#+begin_src emacs-lisp :tangle yes
